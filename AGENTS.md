...
- **豁免 · 限流 / 并发闸**：护成本入口硬拒（如 `RATE_LIMITED` / `CONCURRENT_LIMITED`）**可以不落库**——刷接口时每条都写会话会把历史刷脏；与空库那类「产品态拒答」不同。usecase 入口 `raise AppError` → HTTP 429/503 即可。若以后要「续聊不断档」，再单独做「已有 `conversation_id` 才落用户问 + 助手提示」，勿与本豁免混成两套口径。
